...
### Step 5: Configure the Firecrawl API Key (web research)

The researcher agent can read live web pages through [Firecrawl](https://firecrawl.dev/). Firecrawl keys start with `fc-`; add yours to the same `.env` file:

```
FIRECRAWL_API_KEY=fc_your_firecrawl_key_here
```

Without a key, runs still work — the researcher simply gets no web content and one note is logged. Use `--no-web` (CLI) or untick **Web research** (GUI) to skip it even when a key is set.
...
Disable Firecrawl web research for a run:
```powershell
.venv\Scripts\python main.py --no-web "Research how capacitors work and write a summary"
```
...
- **Web research** (ticked by default) sends researcher steps through Firecrawl; it is greyed out in Mock mode. See section 8.
...
## 8. Web Research with Firecrawl (Researcher Agent)

The researcher agent can read live web pages. `firecrawl_client.py` is a small Firecrawl v2 client (standard library only — nothing extra to install) and `research.py` wraps the *injected* `llm` callable, so the orchestrator, executor, reviewer and pipeline stay untouched:

1. The orchestrator hands a step to the researcher exactly as before.
2. Before that prompt reaches the model, the wrapper:
   - **scrapes** the URLs named in the step instruction when there are any (up to `FIRECRAWL_MAX_URLS`, default 2), or
   - **searches** the web with the step instruction when there is no URL (`FIRECRAWL_SEARCH_LIMIT`, default 3 results, fetched as markdown).
3. The scraped markdown is appended to the researcher's prompt as a `--- Web research (Firecrawl) ---` section with the source URLs, so findings can be traced back to a page.
4. Only researcher steps are enriched; writer/tester/reviewer prompts pass through unchanged. Fetched pages are cached per run, so a review round re-runs the step without spending more Firecrawl credits.
5. If `FIRECRAWL_API_KEY` is missing or Firecrawl fails, the run continues with the plain prompt and logs one line (`WEB    -> web research skipped ...`). Live runs print the `WEB` lines; the GUI shows them in blue.

Knobs live in `config.py`:

| Setting | Default | Meaning |
| --- | --- | --- |
| `FIRECRAWL_MAX_URLS` | `2` | How many URLs found in a step prompt get scraped |
| `FIRECRAWL_SEARCH_LIMIT` | `3` | Search results used when the step names no URL |
| `FIRECRAWL_MAX_CHARS` | `6000` | Total scraped markdown added to the prompt |
| `FIRECRAWL_TIMEOUT` | `60.0` | Seconds per Firecrawl request |

Both modules are covered by offline tests (`tests/test_firecrawl.py`, `tests/test_research.py`) using an injected fake HTTP transport and a fake Firecrawl client, so the suite needs no key and no network:
