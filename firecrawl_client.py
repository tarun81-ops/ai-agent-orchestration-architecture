...
needed and nothing extra has to be bundled for the PyInstaller build. The HTTP
transport is injectable, which keeps the pytest suite offline.
...
def _urlopen_transport(url: str, body: bytes, headers: dict[str, str], timeout: float) -> tuple[int, bytes]:
...
class FirecrawlClient:
    ...
    def api_key(self) -> str:
        ...
        raw = self._api_key if self._api_key is not None else os.environ.get(FIRECRAWL_KEY_ENV, "")
        key = (raw or "").strip()
        if not key:
    ...
    def search(self, query: str, limit: int = 3, with_content: bool = True) -> list[WebPage]:
